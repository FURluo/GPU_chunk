...
public final class NoiseCache {
    ...
    private int mask;
    private int size;
    /** 扩容阈值 = capacity / 2（保证 load factor <= 0.5），随扩容同步增长。
     *  修复：旧实现用固定的 maxSize 做阈值，扩容后 size 仍 > maxSize，
     *  导致每次插入都触发全表 rehash，到达上限后每次插入都清空整个缓存。 */
    ...
    // 三角插值会连续 8 次调用同一实例。缓存 lastInstance 的唯一 ID 和
    // 预计算的 hash 基数（instanceId * K），可避免 87% 命中路径中的数组扫描
    // 和 1 次 64 位乘法。
    //
    // 内存安全：lastInstance / instanceRefs 是强引用，但 NoiseCache 通过 ThreadLocal
    // 持有，生命周期与区块工作线程一致；beginChunk() 会清空缓存（含实例引用），
    // 不会长期持有 ImprovedNoise 实例。
    // ------------------------------------------------------------------
    // ------------------------------------------------------------------
    // 实例 ID 分配
    //
    // 修复：旧实现用 System.identityHashCode 作为缓存 key 的实例标识，
    // 但 identityHashCode 不保证唯一，两个不同 ImprovedNoise 实例碰撞时
    // 会静默返回错误噪声值，导致世界生成损坏。
    // 新实现：在缓存生命周期内按引用相等性分配递增的唯一 ID。
    // 同一区块通常只有 1~16 个实例，线性扫描开销可忽略；
    // lastInstance 快速路径覆盖 ~87% 的连续同实例调用。
    // ------------------------------------------------------------------
    private ImprovedNoise[] instanceRefs = new ImprovedNoise[16];
    ...
    private NoiseCache() {
        int target = Math.max(256, Config.cacheSize);
        // 初始容量取 >= 2*target 的最小 2 的幂，保证 load factor <= 0.5
        int cap = 16;
        while (cap < (target << 1)) cap <<= 1;
        this.capacity = cap;
        this.mask = cap - 1;
        this.growThreshold = cap >> 1;
    ...
    /**
     * 解析 instance 的预计算 hash 基数（基于唯一实例 ID，非 identityHashCode）。
     *
     * <p>快速路径：若 instance 与上次相同（三角插值的常见情况），直接复用
     * 预计算的 {@code instanceId * K}。否则在小数组中按引用扫描/登记。</p>
     */
    private long resolveInstancePreHash(ImprovedNoise instance) {
        ...
        for (int i = 0; i < instanceCount; i++) {
            if (instanceRefs[i] == instance) {
                instanceId = i + 1;
                break;
            }
        }
        if (instanceId == 0) {
            if (instanceCount == instanceRefs.length) {
                instanceRefs = Arrays.copyOf(instanceRefs, instanceRefs.length * 2);
            }
            instanceRefs[instanceCount] = instance;
            instanceCount++;
    ...
    private void putRaw(long instanceId, long preHash,
        ...
        clear();
        putRaw(instanceId, preHash, x, y, z, yFloorFreq, yFloorValue, result, isPrefetch);
    }
    ...
    public void clear() {
        ...
        Arrays.fill(fromPrefetch, false);
        size = 0;
        // 释放实例引用并重置 ID 分配（新区块重新登记）
        Arrays.fill(instanceRefs, 0, instanceCount, null);
        instanceCount = 0;
        lastInstance = null;
        lastInstanceId = 0L;
