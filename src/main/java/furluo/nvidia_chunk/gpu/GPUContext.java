 ...
 * </ul>
 *
 * <p>线程模型：{@code cl_context} / {@code cl_program} 可被多线程共享；
 * {@code cl_kernel} 的参数状态不是线程安全的（clSetKernelArg 并发会互相覆盖），
 * 因此每个线程持有独立的 {@code cl_kernel} 副本；{@code cl_command_queue} 与
 * {@code cl_mem} 缓冲区同样为每线程独有（通过 {@link ThreadLocal}）。</p>
 *
...
public class GPUContext {
     ...
     * 每线程原生资源句柄持有器。
     *
     * <p>修复：旧实现的 PhantomReference 直接强引用 PerThreadState 本身，
     * 导致 referent 永远强可达、幽灵引用永不入队，清理线程形同虚设，
     * 工作线程死亡后原生资源泄漏。现在 StateReference 只持有本句柄容器，
     * PerThreadState 被 GC 后即可据此释放原生资源。</p>
     */
    private static final class NativeResources {
        volatile cl_command_queue queue;
        volatile cl_kernel kernel;
        volatile cl_mem permMem;
        volatile cl_mem inputMem;
        volatile cl_mem outputMem;
        boolean released;  // 由 releaseNativeResources 同步保护，防止重复释放
    }
    ...
    private static class StateReference extends PhantomReference<PerThreadState> {
        ...
        StateReference(PerThreadState state, ReferenceQueue<PerThreadState> queue) {
            super(state, queue);
            // 只持有句柄容器，绝不能持有 state 本身（否则幽灵引用永不触发）
        ...
        cleanupThread = new Thread(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    if (ref != null) {
                        releaseNativeResources(ref.resources);
    ...
    private static void logDeviceDetails(cl_device_id device, cl_platform_id platform) {
        try {
            ...
            long[] memBaseAddrAlign = new long[1];
            try {
                clGetDeviceInfo(device, CL_DEVICE_MEM_BASE_ADDR_ALIGN, Sizeof.cl_uint,
                        Pointer.to(memBaseAddrAlign), null);
            } catch (Throwable ignored) {
            }
            sb.append("[nvidia_chunk]   内存对齐: ").append(memBaseAddrAlign[0]).append(" bits\n");
    ...
    public void dispatchBatch(byte[] perm, double xo, double yo, double zo,
        ...
        try {
            ...
            // 使用每线程独立的 kernel：clSetKernelArg 修改的是 kernel 对象状态，
            // 共享 kernel 在多线程并发设参时会互相覆盖（错误结果/崩溃）
            cl_kernel k = state.kernel;
            clSetKernelArg(k, 0, Sizeof.cl_mem, Pointer.to(state.permMem));
            state.scalarArgs[0] = xo;
            clSetKernelArg(k, 1, Sizeof.cl_double, Pointer.to(state.scalarArgs));
            state.scalarArgs[0] = yo;
            clSetKernelArg(k, 2, Sizeof.cl_double, Pointer.to(state.scalarArgs));
            state.scalarArgs[0] = zo;
            clSetKernelArg(k, 3, Sizeof.cl_double, Pointer.to(state.scalarArgs));
            clSetKernelArg(k, 4, Sizeof.cl_mem, Pointer.to(state.inputMem));
            clSetKernelArg(k, 5, Sizeof.cl_mem, Pointer.to(state.outputMem));
            state.countArg[0] = count;
            clSetKernelArg(k, 6, Sizeof.cl_int, Pointer.to(state.countArg));
            ...
            clEnqueueNDRangeKernel(state.queue, k, 1, null,
    ...
    private PerThreadState getPerThreadState() {
        ...
        if (state == null) {
            state = new PerThreadState(context, device, program, workGroupSize);
            perThread.set(state);
            startCleanupThread();
    ...
    private static final class PerThreadState {
        ...
        final cl_kernel kernel;   // 每线程独立 kernel（参数状态非线程安全）
        final int workGroupSize;
        /** 原生句柄镜像，供 PhantomReference 清理使用（不持有 PerThreadState 本身）。 */
        final NativeResources resources = new NativeResources();
        ...
        @SuppressWarnings("deprecation") // clCreateCommandQueue：刻意使用 OpenCL 1.x API 以最大化兼容性
        PerThreadState(cl_context context, cl_device_id device, cl_program program, int workGroupSize) {
            this.context = context;
            ...
            this.kernel = clCreateKernel(program, "improved_noise_kernel", null);
            this.workGroupSize = workGroupSize;
            ...
            // 登记句柄镜像（inputMem/outputMem 在 ensureBuffers 中登记）
            this.resources.queue = this.queue;
            this.resources.kernel = this.kernel;
            this.resources.permMem = this.permMem;
        ...
        void ensureBuffers(int requiredCount) {
            ...
            // 同步句柄镜像（旧句柄已释放，清理时只释放最新句柄）
            this.resources.inputMem = this.inputMem;
            this.resources.outputMem = this.outputMem;
        }
    ...
    public synchronized void close() {
        ...
        try {
            ...
            // 注：不调用 clReleaseDevice —— 它仅适用于 clCreateSubDevices 创建的子设备
    ...
    private static void releaseNativeResources(NativeResources res) {
        synchronized (res) {
            if (res.released) return;  // 防止 cleanup 线程与 close() 重复释放
            res.released = true;
            try {
                if (res.inputMem != null) clReleaseMemObject(res.inputMem);
                if (res.outputMem != null) clReleaseMemObject(res.outputMem);
                if (res.permMem != null) clReleaseMemObject(res.permMem);
                if (res.kernel != null) clReleaseKernel(res.kernel);
                if (res.queue != null) clReleaseCommandQueue(res.queue);
            } catch (Throwable ignored) {
            }
        }
    }
