...
public class NvidiaChunkCommand {
    ...
    private static int debug(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> Component.literal("  prefetchSize: " + Config.PREFETCH_SIZE_INT
                + "（预取" + (Config.prefetchEnabled ? "启用" : "禁用") + "）"), false);
        ...
        if (mgr == null) {
            ...
            for (String line : mgr.getDetailedStats()) {
    ...
    private static int reload(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        Config.reload();
        source.sendSuccess(() -> Component.literal("[nvidia_chunk] 配置已重新加载（注：GPU 设备相关设置需重启才能生效）"), false);
        return 1;
    }
}