...
public final class GPUNoiseManager {
    ...
    private final AtomicLong cpuComputations = new AtomicLong();
    private final AtomicLong gpuBatchDispatches = new AtomicLong();
    private final AtomicLong gpuBatchComputations = new AtomicLong();
    private final AtomicLong prefetchTriggers = new AtomicLong();
    private final AtomicLong prefetchSuccesses = new AtomicLong();
    private final AtomicLong prefetchFails = new AtomicLong();
    private final AtomicLong prefetchPredictedCoords = new AtomicLong();
    private final AtomicLong prefetchHitCoords = new AtomicLong();
    private final AtomicLong chunksSeen = new AtomicLong();
    /** 区块生成耗时统计：最小值（纳秒）。 */
    private final AtomicLong chunkTimeMinNanos = new AtomicLong(Long.MAX_VALUE);
    /** 区块生成耗时统计：最大值（纳秒）。 */
    private final AtomicLong chunkTimeMaxNanos = new AtomicLong(0);
    /** 区块生成耗时统计：累计值（纳秒）。 */
    private final AtomicLong chunkTimeSumNanos = new AtomicLong(0);
    /** 区块生成耗时统计：有效样本数。 */
    private final AtomicLong chunkTimeCount = new AtomicLong(0);
    /** 首个完整区块生成时间样本数（用于快速反馈，前 10 个区块各打一行）。 */
    private final AtomicInteger firstChunkSamples = new AtomicInteger(0);
    ...
    public static synchronized void init(GPUContext gpu) {
        ...
        final int MAX_SAFE_PREFETCH = 256;
        ...
        LOGGER.info("[nvidia_chunk] 配置确认：chunkPrewarm={}, prefetchSize={}, verbose={}, GPU启用={}",
                Config.chunkPrewarm, configuredPrefetchSize, Config.verbose, Config.enabled);
    ...
    public double dispatchNoise(ImprovedNoise instance, byte[] perm,
        ...
        // 线程诊断：检测噪声计算的线程
        if (Config.verbose && totalCalls.get() <= 5) {
        ...
        if (useGpu) {
            try {
                ...
                onGpuFailure(t);
                cpuComputations.incrementAndGet();
        ...
        // 4) 自适应批量预取：观察连续未命中的坐标差值，推测 cell 三轴步长，
        //    步长稳定后用推测的步长预测当前 cell 的 7 个其他顶点 + 沿主轴预测相邻 cell。
        //    预取完全隔离：任何异常都不影响主路径。
        //
        //    触发策略：
        //    - 步长稳定后首次未命中：立即预取一次（不节流），快速预填当前 cell 的其他顶点。
        //    - 后续未命中：按 prefetchSize 节流，避免 GPU 调度开销过高。
        //      每 prefetchSize 次未命中才触发一次预取，一次预取提交 prefetchSize 个坐标。
        //      GPU 调度频率降为 1/(prefetchSize+1)，平衡预取命中率与调度开销。
        // 跨实例批量累加 + 阈值触发 GPU 调度（GPU 加速且 TPS 稳定的核心方案）
        //
        // 设计原理：
        // 1) 缓存未命中时立即用 CPU 计算（~100ns），保证主路径低延迟。
        // 2) 同时将 (perm, xo, yo, zo, x, y, z, yff, yfv) 追加到线程级累加器。
        // 3) 当累加器达到阈值（默认 512 个坐标）时，自动 flush 一次 GPU 大批次。
        // 4) GPU 调度频率由阈值控制：每 chunk 8-16 个实例 × 每实例数十次未命中 →
        //    总计 1-2 次 GPU 调度/chunk，摊薄 ~10ms PCIe 开销后每个坐标均摊约 20μs。
        // 5) cell 角点在相邻 cell 间共享，GPU 预填的缓存会被后续 noise() 调用 100% 命中。
        if (Config.enabled
                && gpu != null
                && gpu.isAvailable()
                && !gpuDisabled) {
            try {
                flushOrAccumulate(instance, perm, xo, yo, zo, x, y, z,
                        yFloorFreq, yFloorValue, buffers -> {
                            int off = buffers.coordCount * 5;
                ...
                // 累加异常：完全静默，不影响主路径
                if (Config.verbose) {
                    LOGGER.warn("[nvidia_chunk] 累加异常（已忽略）：{}", t.toString());
                }
            }
        }
    ...
    /**
     * 累加当前坐标到线程级批处理缓冲区；当缓冲区达到阈值时自动 flush GPU。
     *
     * <p>同 perm 引用（含相同 xo/yo/zo）才能合并到同一批次。
     * 当 instance 变化时，立即 flush 旧批次再开始新批次（不同 perm 表的 GPU 调度需要分开）。</p>
     */
    private void flushOrAccumulate(ImprovedNoise instance, byte[] perm,
                                    double xo, double yo, double zo,
                                    double x, double y, double z,
                                    double yff, double yfv,
                                    java.util.function.Consumer<PrefetchBuffers> appender) {
        PrefetchBuffers buffers = prefetchBuffers.get();

        // 批次大小阈值：超过即触发 GPU 调度
        // 注：这里用 prefetchBuffers.coords 容量/5 作为上限；动态扩容在 flush 中处理。
        final int BATCH_THRESHOLD = Config.PREFETCH_SIZE_INT > 0
                ? Math.min(Config.PREFETCH_SIZE_INT, 1024) : 512;

        // 同一批次要求：相同 instance + 相同 perm 引用 + 相同 xo/yo/zo
        boolean sameBatch = buffers.currentInstance == instance
                && buffers.currentPerm == perm
                && buffers.currentXo == xo
                && buffers.currentYo == yo
                && buffers.currentZo == zo;

        if (!sameBatch && buffers.coordCount > 0) {
            // 实例切换：先 flush 旧批次
            flushBatch(buffers, "实例切换");
            buffers.currentInstance = instance;
            buffers.currentPerm = perm;
            buffers.currentXo = xo;
            buffers.currentYo = yo;
            buffers.currentZo = zo;
        ...
        // 累加当前坐标
        appender.accept(buffers);

        // 达到阈值 → flush
        if (buffers.coordCount >= BATCH_THRESHOLD) {
            flushBatch(buffers, "达到阈值");
        }
    }

    /**
     * 将累加器中的所有坐标一次性提交到 GPU 批量调度，结果写入 NoiseCache。
     */
    private void flushBatch(PrefetchBuffers buffers, String reason) {
        if (buffers.coordCount == 0) return;

        int count = buffers.coordCount;
        ImprovedNoise inst = buffers.currentInstance;
        byte[] perm = buffers.currentPerm;
        double xo = buffers.currentXo, yo = buffers.currentYo, zo = buffers.currentZo;

        // 委托给 prefetchBatch（已有的批量调度路径）
        try {
            int computed = prefetchBatch(inst, perm, xo, yo, zo, buffers.coords, count);
            if (computed > 0) {
                prefetchTriggers.incrementAndGet();
            }
        } catch (Throwable t) {
            prefetchFails.incrementAndGet();
            if (Config.verbose) {
                LOGGER.warn("[nvidia_chunk] flushBatch 异常（{}，{}个坐标）：{}", reason, count, t.toString());
            }
        }

        // 清空累加器
        buffers.coordCount = 0;
    }
    ...
    private void speculativePrefetch(ImprovedNoise instance, byte[] perm,
        ...
        // 最大 512 个坐标（约 8KB 数据），超过则截断
        // 这样单次 GPU 调度不会占用太多时间
        final int MAX_SAFE_PREFETCH = 512;
    ...
    public int prefetchBatch(ImprovedNoise instance, byte[] perm,
        ...
        try {
            ...
            prefetchPredictedCoords.addAndGet(filtered);
            ...
            try {
                ...
                gpuBatchDispatches.incrementAndGet();
                gpuBatchComputations.addAndGet(filtered);
                consecutiveFailures = 0;
                gpuSuccess = true;
            } catch (Throwable t) {
                onGpuFailure(t);
                // GPU 失败：CPU 回退逐个计算
                cpuComputations.addAndGet(filtered);
            ...
            if (Config.verbose || gpuBatchDispatches.get() <= 5) {
            ...
            prefetchSuccesses.incrementAndGet();
            return filtered;
        } catch (Throwable t) {
            // 最外层兜底：任何未预期的异常都静默
            if (Config.verbose) {
    ...
    private static final class PrefetchBuffers {
        /** GPU 批处理累加器：缓存未命中坐标（按 5 个 double 为一组：x, y, z, yff, yfv）。 */
        double[] coords = new double[1024 * 5];
        /** 累加器中有效组数（每组 5 个 double）。 */
        int coordCount;
        /** 当前累加器所属的 ImprovedNoise 实例。null = 当前批次尚未启动。 */
        ImprovedNoise currentInstance;
        /** 当前批次的 perm 引用。 */
        byte[] currentPerm;
        /** 当前批次的 xo/yo/zo。 */
        double currentXo, currentYo, currentZo;
        /** 自上次预取以来的未命中次数（用于节流）。 */
        long missCountSinceLastPrefetch;
        /** 是否已经预取过（每个 chunk 限 1 次以保护 TPS）。 */
        ...
        // 区块生成时间统计（Advisor 方案）
        /** 上一次 beginChunk 的时间戳（纳秒）。0 = 未初始化。 */
        long lastChunkStartNanos;
        /** 该线程已处理的区块数（用于在 beginChunk 时计算上一个区块的耗时）。 */
        int threadChunkCount;

        // 步长观察字段（保留兼容，但新方案不依赖于此）
    ...
    public void beginChunk() {
        ...
        // 区块生成时间统计（Advisor 方案）
        // 测量"上一次 beginChunk 到本次 beginChunk"的时间差，即上一区块的耗时。
        // 这是合理的近似：包含了上一区块的 noise() 调用、GPU flush、缓存写入等所有工作。
        long nowNanos = System.nanoTime();
        if (buffers.lastChunkStartNanos != 0) {
            long elapsedNanos = nowNanos - buffers.lastChunkStartNanos;
            // 仅记录合理的耗时（>0.1ms 且 < 60s），过滤异常
            if (elapsedNanos >= 100_000L && elapsedNanos <= 60_000_000_000L) {
                // 更新最小值（CAS 循环）
                long cur;
                do { cur = chunkTimeMinNanos.get(); }
                while (elapsedNanos < cur && !chunkTimeMinNanos.compareAndSet(cur, elapsedNanos));
                // 更新最大值
                do { cur = chunkTimeMaxNanos.get(); }
                while (elapsedNanos > cur && !chunkTimeMaxNanos.compareAndSet(cur, elapsedNanos));
                chunkTimeSumNanos.addAndGet(elapsedNanos);
                long count = chunkTimeCount.incrementAndGet();

                // 前 10 个区块的耗时逐个输出（快速反馈）
                int sampleIdx = firstChunkSamples.getAndIncrement();
                if (sampleIdx < 10) {
                    LOGGER.info("[nvidia_chunk] [区块耗时 #{}] 本区块耗时: {}ms",
                            sampleIdx + 1, String.format("%.2f", elapsedNanos / 1_000_000.0));
                }
            }
        }
        buffers.lastChunkStartNanos = nowNanos;
        buffers.threadChunkCount++;

        // 关键：beginChunk 时先 flush 上一 chunk 残留的累加器，
        // 确保未提交的坐标也走 GPU 路径（不浪费已计算的数据）
        if (buffers.coordCount > 0) {
            flushBatch(buffers, "beginChunk");
        }
    ...
    private void logStats() {
        ...
        if (!loggedFirstChunk) {
            ...
            LOGGER.info("[nvidia_chunk] 总调用: {}, 缓存命中: {}, 命中率: {}%",
                    total, cacheHits.get(),
                    String.format("%.1f", (double) cacheHits.get() / total * 100));
            LOGGER.info("[nvidia_chunk] CPU计算: {}, GPU批次数: {}, GPU计算数: {}",
                    cpuComputations.get(), gpuBatchDispatches.get(), gpuBatchComputations.get());
            LOGGER.info("[nvidia_chunk] 预取触发: {}, 预取成功: {}, 预取失败: {}",
                    prefetchTriggers.get(), prefetchSuccesses.get(), prefetchFails.get());
            long prefetchPuts = prefetchPredictedCoords.get();
            if (prefetchPuts > 0) {
                NoiseCache cache = NoiseCache.get();
                long prefetchHits = cache.getPrefetchHits();
                double prefetchHitRate = (double) prefetchHits / prefetchPuts * 100;
                LOGGER.info("[nvidia_chunk] 预取预测: {}, 预取命中: {}, 预取命中率: {:.1f}%",
                        prefetchPuts, prefetchHits, prefetchHitRate);
            }
            if (gpuBatchDispatches.get() == 0) {
        ...
        if (now - lastStatsLogTime >= 60000) {
            ...
            double avgPrefetchPerChunk = chunksSeen.get() > 0
                    ? (double) prefetchTriggers.get() / chunksSeen.get() : 0;
            String mode = (Config.PREFETCH_SIZE_INT <= 0 || gpu == null || !gpu.isAvailable() || gpuDisabled)
                    ? "纯缓存（预取已禁用）"
                    : "GPU预取模式";
            // 区块生成耗时统计
            long chunkCount = chunkTimeCount.get();
            double minMs = chunkCount > 0 ? chunkTimeMinNanos.get() / 1_000_000.0 : 0;
            double maxMs = chunkCount > 0 ? chunkTimeMaxNanos.get() / 1_000_000.0 : 0;
            double avgMs = chunkCount > 0 ? (chunkTimeSumNanos.get() / (double) chunkCount) / 1_000_000.0 : 0;
            LOGGER.info("[nvidia_chunk] 统计（近60秒）[{}]: 区块={}, 总调用={}, 命中率={}%, CPU={}, GPU批={}, GPU计算={}, 预取触发={} (平均/chunk={}), 预取成功={}, 预取失败={} | 区块耗时: 最快={}ms, 最慢={}ms, 平均={}ms",
                    mode, chunksSeen.get(), total, String.format("%.1f", hitRate),
                    cpuComputations.get(), gpuBatchDispatches.get(), gpuBatchComputations.get(),
                    prefetchTriggers.get(), String.format("%.2f", avgPrefetchPerChunk),
                    prefetchSuccesses.get(), prefetchFails.get(),
                    String.format("%.2f", minMs), String.format("%.2f", maxMs), String.format("%.2f", avgMs));
        }
    }
    ...
    public void logDetailedStats() {
        ...
        long cpu = cpuComputations.get();
        long gpuBatch = gpuBatchDispatches.get();
        long gpuCompute = gpuBatchComputations.get();
        long prefetches = prefetchTriggers.get();
        long prefetchSuccess = prefetchSuccesses.get();
        long prefetchFail = prefetchFails.get();

        double hitRate = total > 0 ? (double) hits / total * 100 : 0;
        double gpuRatio = (cpu + gpuCompute) > 0 ? (double) gpuCompute / (cpu + gpuCompute) * 100 : 0;

        LOGGER.info("[nvidia_chunk] ===== 详细统计 ===== ");
        LOGGER.info("[nvidia_chunk] 总调用: {}, 缓存命中: {}, 命中率: {:.1f}%", total, hits, hitRate);
        LOGGER.info("[nvidia_chunk] CPU计算: {}, GPU计算: {}, GPU占比: {:.1f}%", cpu, gpuCompute, gpuRatio);
        LOGGER.info("[nvidia_chunk] GPU批次数: {}", gpuBatch);
        LOGGER.info("[nvidia_chunk] 预取触发: {}, 成功: {}, 失败: {}", prefetches, prefetchSuccess, prefetchFail);

        if (gpuBatch > 0) {
            LOGGER.info("[nvidia_chunk] 平均每批: {:.1f} 次计算", (double) gpuCompute / gpuBatch);
        }

        if (prefetches > 0) {
            LOGGER.info("[nvidia_chunk] 预取成功率: {:.1f}%", (double) prefetchSuccess / prefetches * 100);
        }

        if (gpuDisabled) {
            LOGGER.info("[nvidia_chunk] GPU 已被禁用（连续失败过多）");
        } else if (gpu == null) {
            LOGGER.info("[nvidia_chunk] GPU 未初始化");
        } else if (!gpu.isAvailable()) {
            LOGGER.info("[nvidia_chunk] GPU 不可用");
        } else {
            LOGGER.info("[nvidia_chunk] GPU 状态正常：{} ({})", gpu.getDeviceName(), gpu.getVendorName());
        }
        LOGGER.info("[nvidia_chunk] =====================");
    }
}
